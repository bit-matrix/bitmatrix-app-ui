...
export const Swap2 = (): JSX.Element => {
  ...
  const [quote, setQuote] = useState<{ asset?: PAsset; amount: string; way: SWAP_WAY }>({
    asset: lbtcAsset,
    amount: '',
    way: SWAP_WAY.FROM,
  });

  const [token, setToken] = useState<{ asset?: PAsset; amount: string; way: SWAP_WAY }>({
    asset: undefined,
    amount: '',
    way: SWAP_WAY.TO,
  });
  ...
  const onChangeFromInput = useCallback(
    (current_pool: Pool, pool_config: BmConfig, input: string) => {
      ...
      if (current_pool && pool_config) {
        ...
        if (output.amount > 0) {
          if (quote.way === SWAP_WAY.FROM) {
            setQuote({ ...quote, amount: input });
            setToken({ ...token, amount: (output.amount / PREFERRED_UNIT_VALUE.LBTC).toString() });
            setAmountWithSlippage(output.amountWithSlipapge / PREFERRED_UNIT_VALUE.LBTC);
          } else {
            setToken({ ...token, amount: input });
            if (quote.asset?.ticker === SWAP_ASSET.LBTC) {
              setQuote({ ...quote, amount: (output.amount / settingsContext.preferred_unit.value).toString() });
        ...
        if (output.amount > 0) {
          if (quote.way === SWAP_WAY.FROM) {
            ...
            if (quote.asset?.ticker === SWAP_ASSET.LBTC) {
              ...
              setQuote({ ...quote, amount: (output.amount / PREFERRED_UNIT_VALUE.LBTC).toFixed(2) });
              setAmountWithSlippage(output.amountWithSlipapge / PREFERRED_UNIT_VALUE.LBTC);
            }
          } else {
            setQuote({ ...quote, amount: input });
            setToken({ ...token, amount: (output.amount / PREFERRED_UNIT_VALUE.LBTC).toFixed(2) });
          }
        } else {
          if (quote.way === SWAP_WAY.FROM) {
            setToken({ ...token, amount: '' });
  ...
  const calcAmountPercent = useCallback(
    (newFromAmountPercent: FROM_AMOUNT_PERCENT | undefined, balances: Balance[]) => {
      if (token && poolsContext.length > 0 && poolConfig && walletContext && balances.length > 0) {
        let inputAmount = '';
        let isQuote = true;

        let who = quote;
        if (token.way === SWAP_WAY.FROM) {
          who = token;
          isQuote = false;
        }

        const totalAmountInWallet = balances.find((bl) => bl.asset.assetHash === who.asset?.assetHash)?.amount || 0;

        if (who.asset?.ticker === SWAP_ASSET.LBTC) {
          const totalFee =
          ...
          setQuote({ ...quote, amount: inputAmount });
        } else {
          if (newFromAmountPercent === FROM_AMOUNT_PERCENT.ALL) {
          ...
          if (isQuote) {
            ...
            setToken({ ...token, amount: inputAmount });
          }
        }
    ...
    [currentPool, poolConfig, settingsContext, walletContext],
  );
  ...
  const inputIsValid = useCallback(() => {
    if (currentPool && poolsContext.length > 0 && poolConfig && walletContext) {
      ...
      const inputFromValue = Number(who.asset?.value);

      if (inputFromValue > 0) {
        const pair1AssetHash = who.asset?.assetHash;
  ...
  const swapRouteChange = () => {
    ...
    setQuote({ ...token, amount: '' });
    setToken({ ...quote, amount: '' });
    // setInputFromAmount('');
    // setInputToAmount('');
    setSelectedFromAmountPercent(undefined);
  };
              ...
              <div className="from-input-content">
                <div className="from-amount-div">
                  ...
                  <NumericalInput
                    ...
                    inputValue={quote.way === SWAP_WAY.FROM ? quote.amount : token.amount}
                    onChange={(inputValue) => {
                      if (inputValue === '.') return;
                      if (quote.way === SWAP_WAY.FROM) {
                        setQuote({ ...quote, amount: inputValue });
                      } else {
                        setToken({ ...token, amount: inputValue });
                      }
                    ...
                    decimalLength={getAssetPrecession(
                      quote.way === SWAP_WAY.FROM
                        ? (quote.asset?.ticker as SWAP_ASSET)
                        : (token.asset?.ticker as SWAP_ASSET),
            ...
            <div className="from-content">
              <div className="from-input-content">
                <div className="from-amount-div">
                  ...
                  <NumericalInput
                    className="from-input"
                    inputValue={token.way === SWAP_WAY.TO ? token.amount : quote.amount}
                    onChange={(inputValue) => {
                      if (inputValue === '.') return;
                      if (token.way === SWAP_WAY.TO) {
                        setToken({ ...token, amount: inputValue });
                      } else {
                        setQuote({ ...quote, amount: inputValue });
                      }
